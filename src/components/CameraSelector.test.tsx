...
describe("CameraSelector", () => {
  ...
  it("shows error when camera permissions are denied", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText(
          "Unable to access cameras. Please ensure you have granted camera permissions."
        )
      ).toBeInTheDocument();
    });
  });
  ...
  describe("Component lifecycle and cleanup", () => {
    it("does not update state after unmounting", async () => {
      ...
      let resolveGetUserMedia: (
        value: MediaStream | PromiseLike<MediaStream>
      ) => void = () => {};
      ...
      resolveGetUserMedia(new MediaStream());

      // Should not throw any errors about setting state on unmounted component
      await waitFor(() => {
        expect(
          screen.queryByText("SCANNING DEVICES...")
    ...
    it("displays corner decorations correctly", async () => {
      ...
      await waitFor(() => {
        ...
        const corners = container?.querySelectorAll(
          'div[class*="absolute"][class*="border"]'
        );
  ...
  describe("Security and permissions", () => {
    ...
    it("does not expose device IDs in error messages", async () => {
      const sensitiveError = new Error(
        "Device camera-secret-id-12345 not found"
      );
      ...
      await waitFor(() => {
        ...
        expect(
          screen.queryByText(/camera-secret-id-12345/)
        ).not.toBeInTheDocument();
      });
    });
  });
  ...
  describe("Memory leaks and performance", () => {
    ...
    it("handles memory pressure scenarios", async () => {
      ...
      const mockDevices = Array.from({ length: 100 }, (_, i) => ({
        ...
        label: `Camera ${
          i + 1
        } with a very long label that might cause layout issues or memory problems`,
      }));
  ...
  describe("Concurrent operations and race conditions", () => {
    it("handles component unmounting during API calls", async () => {
      let rejectGetUserMedia: (reason: Error) => void;
